...
def strategy1(data, stock_name):
    ...
    ma1 = Indicater.calculate_sma1(data,window=16)
    ma2 = Indicater.calculate_sma2(data,window=21)

    data['sma1'] = data['close'].rolling(window=16).mean()
    data['sma2'] = data['close'].rolling(window=25).mean()

    adx1 = Indicater.calculate_adx(data, window=27)
    atr1 = Indicater.calculate_atr(data, window=3)
    choppyindex = Indicater.calculate_choppiness_index(data, length=22)
    My_roc = Indicater.calculate_roc(data,window=14)
    ...
    for i in range(1, len(data)):
        ...
        if Flag < 1:
            # Check for MA crossover, ADX, ATR, and ChoppinessIndex conditions
            if (My_roc['roc'].iloc[i] > 0 and My_roc['roc'].iloc[i-1] < 0 ):
                LongSignal = 1


        if Flag > -1:
            # Check for MA crossover in the opposite direction, ADX, ATR, and ChoppinessIndex conditions
            if (My_roc['roc'].iloc[i] < 0 and My_roc['roc'].iloc[i-1] > 0):
                ShortSignal = 1

        # if Flag < 1:
        #     # Check for MA crossover, ADX, ATR, and ChoppinessIndex conditions
        #     if data['time'].iloc[i] == 2100:
        #         LongSignal = 1
        #
        # if Flag > -1:
        #     # Check for MA crossover in the opposite direction, ADX, ATR, and ChoppinessIndex conditions
        ...
        if LongSignal == 1 and Flag != 1 and LongTrade < AllowedTradesLong and TradesTaken < AllowedTrades and (
        ...
        if ShortSignal == 1 and Flag != -1 and ShortTrade < AllowedTradesShort and TradesTaken < AllowedTrades and (
...
def main():
    ...
    percentage_cost = 0.00 #float(input("Enter the percentage cost (e.g., 0.001 for 0.1%): "))
