...
printtradesheet = 1
csv_folder_path = r"C:\Users\ADMIN\PycharmProjects\Backtest_engine\Data_script"
output_tradesfile_path = r"C:\Users\ADMIN\PycharmProjects\Backtest_engine\Tradesheet"
...
def analyze_trades(csv_file, output_excel, percentage_cost=0.0125, per_unit_cost=1.5):
    trades_df = pd.read_csv(csv_file)
    trades_df['PercentageCost'] = trades_df['entry_price'] * percentage_cost/100 * abs(trades_df['qty'])
    ...
    daily_profit = trades_df.groupby(trades_df['entry_date'].dt.strftime('%Y%m%d')).agg(DailyMTM=('MTM', 'sum')).reset_index()
    ...
    monthly_profit = trades_df.groupby(trades_df['entry_month']).agg(MonthlyMTM=('MTM', 'sum')).reset_index()
    ...
    yearly_profit = trades_df.groupby(trades_df['entry_year']).agg(YearlyMTM=('MTM', 'sum')).reset_index()
    ...
    with pd.ExcelWriter(output_excel) as writer:
        trades_df[['entry_price', 'exit_price', 'qty', 'MTM', 'CumulativeMTM']].to_excel(writer, sheet_name='Trade Analysis', index=False)
        ...
        analysis_df = pd.DataFrame({
            'Metric': ['Net profit', 'MAX DD', 'CALMA ratio', 'Max qty', 'Gross Profit', 'Gross Loss', 'Profit Factor', 'Day Max Profit', 'Day Max Loss', 'Total Trades', 'Max daily DD', 'Max monthly DD', 'Max Yearly DD', 'Average Daily Profit', 'Average Monthly Profit', 'Average Yearly Profit'],
            'All Trades': [all_metrics.get(m) for m in ['Net profit', 'MAX DD', 'CALMA ratio', 'Max qty', 'Gross Profit', 'Gross Loss', 'Profit Factor', 'Day Max Profit', 'Day Max Loss', 'Total Trades', 'Max daily DD', 'Max monthly DD', 'Max Yearly DD', 'Average Daily Profit', 'Average Monthly Profit', 'Average Yearly Profit']],
            'Long Trades': [long_metrics.get(m) for m in ['Net profit', 'MAX DD', 'CALMA ratio', 'Max qty', 'Gross Profit', 'Gross Loss', 'Profit Factor', 'Day Max Profit', 'Day Max Loss', 'Total Trades', 'Max daily DD', 'Max monthly DD', 'Max Yearly DD', 'Average Daily Profit', 'Average Monthly Profit', 'Average Yearly Profit']],
            'Short Trades': [short_metrics.get(m) for m in ['Net profit', 'MAX DD', 'CALMA ratio', 'Max qty', 'Gross Profit', 'Gross Loss', 'Profit Factor', 'Day Max Profit', 'Day Max Loss', 'Total Trades', 'Max daily DD', 'Max monthly DD', 'Max Yearly DD', 'Average Daily Profit', 'Average Monthly Profit', 'Average Yearly Profit']]
        })
...
def main():
    ...
    if uploaded_files:
        percentage_cost = st.number_input("Percentage Cost", value=0.000125)
        per_unit_cost = st.number_input("Per Unit Cost", value=1.5)
        ...
        for uploaded_file in uploaded_files:
            ...
            stock_name = os.path.splitext(uploaded_file.name)[0]
            tradesheet = strategy1(df, stock_name)

            tradesheet_filename = f"{stock_name}_tradesheet.csv"
            tradesheet_filepath = os.path.join(output_dir, tradesheet_filename)
            tradesheet.to_csv(tradesheet_filepath, index=False)

            # Analyze trades
            output_excel = os.path.join(output_dir, f"{stock_name}_analysis.xlsx")
            analyze_trades(tradesheet_filepath, output_excel, percentage_cost, per_unit_cost)


            st.write(f"Processed {uploaded_file.name} and saved analysis to {output_excel}")

            # Display the analysis results
            st.subheader(f"Analysis for {uploaded_file.name}")
            analysis_df = pd.read_excel(output_excel, sheet_name='Analysis')
            st.dataframe(analysis_df)

            # Display individual sheets
            st.subheader("Trade Analysis")
            trade_analysis_df = pd.read_excel(output_excel, sheet_name='Trade Analysis')
            st.dataframe(trade_analysis_df)

            st.subheader("Daily Profit")
            daily_profit_df = pd.read_excel(output_excel, sheet_name='Daily Profit')
            st.dataframe(daily_profit_df)

            st.subheader("Monthly Profit")
            monthly_profit_df = pd.read_excel(output_excel, sheet_name='Monthly Profit')
            st.dataframe(monthly_profit_df)

            st.subheader("Yearly Profit")
            yearly_profit_df = pd.read_excel(output_excel, sheet_name='Yearly Profit')
            st.dataframe(yearly_profit_df)
